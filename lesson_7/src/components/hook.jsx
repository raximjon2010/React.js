    ...
    return (
        <div>
          ...
          </select>
          {
            Data.map((value) => {
                return (
                    <h1 key={value.id}>Id:{value.id} Name:{value.name} <button onClick={() => onDelete(value.id)}>delete</button></h1>
                )
            })
          }
        </div>
...
// Bizda class compoentsda setStateni ikinchi parametri agar state bitta orqada qolib ketsa ishlatilardi 
